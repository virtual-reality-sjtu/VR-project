...
*/     
  ...
  //Calculating nDotL
  float nDotL0 = dot(normal, position0);
  float nDotL1 = dot(normal, position1);
  
  //Calculating the reflection vector
  vec3 reflect0 = normalize((2 * normal * nDotL0) - position0);
  vec3 reflect1 = normalize((2 * normal * nDotL1) - position1);

  //Since the reflection vector and the eye vector are both
  //unit vectors, their dot product will be cos0
  float cos0 = dot(eyedirn, reflect0);
  float cos1 = dot(eyedirn, reflect1);

  //Calculating colors
  vec3 colors0 = vec3(0, 0, 0);
  vec3 colors1 = vec3(0, 0, 0);  

  colors0.x = W.x * pow((cos0 + 1)/2.0, P.x);
  colors0.y = W.y * pow((cos0 + 1)/2.0, P.y);
  colors0.z = W.z * pow((cos0 + 1)/2.0, P.z);

  colors1.x = W.x * pow((cos1 + 1)/2.0, P.x);
  colors1.y = W.y * pow((cos1 + 1)/2.0, P.y);
  colors1.z = W.z = pow((cos1 + 1)/2.0, P.y);

  gl_FragColor = vec4(colors0 + colors1, 1);   
}
