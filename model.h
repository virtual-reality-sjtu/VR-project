...
- You do not have to know what is going on here
...
public:
	...
	// So that I can make primative models manually
...
private:
    ...
    Mesh processMesh(aiMesh* mesh, const aiScene* scene)
    {
		...
		if (mesh->mMaterialIndex >= 0)
		{
			...
			// Material Diffuse Color if it has one
			aiColor4D diffuse;
		...
		Mesh t(vertices, indices, textures);
	...
	GLint TextureFromFile(const char* path, std::string directory, bool gamma = false)
	{
		...
		// Load Image With Free Image
		FIBITMAP* imagen = FreeImage_Load(FreeImage_GetFileType(filename.c_str()), filename.c_str());
		...
		glTexImage2D(GL_TEXTURE_2D, 0, gamma ? GL_SRGB : GL_RGBA, w, h, 0, GL_BGRA, GL_UNSIGNED_BYTE, (void*)FreeImage_GetBits(imagen));
